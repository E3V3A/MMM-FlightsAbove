/* --------------------------------------------------------------------------
 * FileName:     radar.js
 * Author:       Jannis Redmann "derhuerst"
 * Modified:     E:V:A
 * License:      ISC
 * Date:         2018-03-13
 * Version:      0.2.2 -- A modified fork of 0.2.1 (flightradar24-client)
 * Description:  The radar() function with comments and a few mods
 * Format:       4-space TAB's (no TAB chars), mixed quotes
 *
 * URL:          https://github.com/E3V3A/MMM-FlightsAbove
 * Original:     https://github.com/derhuerst/flightradar24-client
 * --------------------------------------------------------------------------
 * Requires:
 *      "dot-prop"
 *      "fetch-ponyfill"
 ...
 * NOTE:
 *
 * This API is based on the 0.2.1 version of the "flightradar24-client".
 * However, it was missing handling several F24 items at the time, so this
 * was modified to include them in the simplest possible way. (No Camels!)
 * Since, then the API has been improved to v0.3.0 with handling missing
 * items, but less organized in structure and with different names.
 *
 * Here we keep things as they are.
 * --------------------------------------------------------------------------
 */
